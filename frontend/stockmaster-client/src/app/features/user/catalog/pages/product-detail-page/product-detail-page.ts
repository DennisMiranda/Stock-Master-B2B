...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, LucideAngularModule,  PrimaryButton, CardPriceVariant],
...
export class ProductDetailPage implements OnInit {
  ...
  // Categories Data
  categories = toSignal(this.categoryService.getCategoriesWithSubcategories(), {
    initialValue: [],
  });
  ...
  getStock(variant: Variant): number {
    const p = this.product();
    if (!p) return 0;
    return variant === 'unit' ? p.stockUnits || 0 : p.stockBoxes || 0;
  }
  ...
  addToCart() {
    ...
    this.variantQuantities().forEach((qty, variant) => {
      if (qty > 0) {
        ...
        const maxQuantity = variant === 'unit' ? product.stockUnits ?? 0 : product.stockBoxes ?? 0;
