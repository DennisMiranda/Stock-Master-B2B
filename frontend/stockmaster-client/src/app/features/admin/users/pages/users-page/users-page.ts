...
import { Check, LucideAngularModule, MoreHorizontal, Plus, Search, X } from 'lucide-angular';
...
import { SortEvent, TableColumn } from '../../../../../shared/ui/data-table/models/table.model';
...
export class UsersPage implements AfterViewInit {
  ...
  totalUsers = computed(() => this.userService.metadata().count);
  activeUsersCount = computed(() => this.users().filter((u: User) => u.isActive).length);
  page = this.userService.page;
  totalPages = computed(() => this.userService.metadata().pages);
